...
def total():
    ...
    #query2
    query=("select sum(amount) from expensetracker2024")
...
def max_spent():
    ...
    #query12
    query=("""SELECT date_format(date, '%y-%m' ) as months, MAX(AMOUNT) as max_amount
            FROM EXPENSETRACKER2024 
            GROUP BY date_format(date, '%y-%m' )
           """)
    df=fetch_data(query)
    ...
    #query13
    query=("""select category, max(amount) , date_format(date, '%y-%m' ) as months from expensetracker2024
...
def min_spent():
    ...
    #query16
    query=("""SELECT date_format(date, '%y-%m' ) as months, MIN(AMOUNT) as max_amount
            FROM EXPENSETRACKER2024 
            GROUP BY date_format(date, '%y-%m' )
           """)
    df=fetch_data(query)
    ...
    #query17
    query=("""select category, min(amount) , date_format(date, '%y-%m' ) as months from expensetracker2024
            group by category, date_format(date, '%y-%m' )
            order by category desc;
           """)
    df=fetch_data(query)
